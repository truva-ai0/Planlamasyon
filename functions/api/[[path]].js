...
export async function onRequest(context) {
  ...
  try {
    ...
    const result = await mod.handler(event, { clientContext: null, cloudflareEnv: context.env });
    return fromNetlifyResult(result);
  } catch (error) {
    console.error('Cloudflare API adapter error', route, error);
    return json(Number(error?.statusCode) || 500, { ok: false, code: error?.code || 'CLOUDFLARE_ADAPTER_ERROR', message: Number(error?.statusCode) >= 400 && Number(error?.statusCode) < 500 ? String(error?.message || 'İstek tamamlanamadı.') : 'Sunucu işlemi tamamlanamadı.' });
  }
}
