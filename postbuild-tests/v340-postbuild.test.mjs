...
test('v3.4.0 kadastro kaydı ile imar hakkını açıkça ayırır', async () => {
...
test('v3.4.0 mobil karar özeti ve doğrulanmayan metrikleri gizleme sözleşmesini taşır', async () => {
  const html = await readFile('dist/index.html', 'utf8');
  const app = await readFile('dist/app.js', 'utf8');
...
test('v3.4.0 hızlı ilk faz otomatik Plan AI beklemez ve süreyi sınırlar', async () => {
...
test('v3.4.0 harita tabanı OSM hatasında uyduya bir kez geçer ve yeniden denetir', async () => {
  ...
  assert.match(app, /switchToImageryFallback/);
  assert.match(app, /mapBaseRetryButton/);
  assert.match(html, /id="mapBaseStatus"/);
});

test('v3.4.0 Beşiktaş yönlendirmesi manuel ve izin koşullarıyla kayıtlıdır', async () => {
...
test('v3.4.0 Beşiktaş yönlendirmesi hiçbir otomatik ağ çağrısı yapmaz', async () => {
  let networkCalls = 0;
  ...
  assert.equal(service?.url, 'https://besiktas.bel.tr/');
  assert.equal(service?.termsUrl, 'https://keos.besiktas.bel.tr/imardurumu/legal.aspx');
});

test('v3.4.0 sürüm ve önbellek anahtarları tek sürümdür', async () => {
  const html = await readFile('dist/index.html', 'utf8');
  const pkg = JSON.parse(await readFile('package.json', 'utf8'));
  assert.equal(pkg.version, '3.4.0');
  assert.match(html, /styles\.css\?v=3\.4\.0/);
  assert.match(html, /app\.js\?v=3\.4\.0/);
  assert.match(html, /PLANLAMASYON · v3\.4\.0/);
});
