    ...
    try {
      ...
      const response = await fetchImpl(request.url, {
        ...
        headers: {
          ...
          'User-Agent': clean(env.OVERPASS_USER_AGENT, 240) || 'Planlamasyon/3.4.0 (+https://planlamasyon.truva-ai.com)',
    ...
    try {
      ...
      const response = await fetchImpl(url, {
        headers: {
          ...
          'User-Agent': clean(env.NOMINATIM_USER_AGENT, 240) || 'Planlamasyon/3.4.0 (+https://planlamasyon.truva-ai.com)'
        },
        signal: controller.signal
      });
